...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	vec4 ext = vec4(1,0,0,1);

    number fac = 0.8 + 0.9*sin(11.*uv.x+4.32*uv.y + graded.r*12. + cos(graded.r*5.3 + uv.y*4.2 - uv.x*4.));
    number fac2 = 0.5 + 0.5*sin(8.*uv.x+2.32*uv.y + graded.r*5. - cos(graded.r*2.3 + uv.x*8.2));
    number fac3 = 0.5 + 0.5*sin(10.*uv.x+5.32*uv.y + graded.r*6.111 + sin(graded.r*5.3 + uv.y*3.2));
    number fac4 = 0.5 + 0.5*sin(3.*uv.x+2.32*uv.y + graded.r*8.111 + sin(graded.r*1.3 + uv.y*11.2));
    number fac5 = sin(0.9*16.*uv.x+5.32*uv.y + graded.r*12. + cos(graded.r*5.3 + uv.y*4.2 - uv.x*4.));
    ...
    hslt.xyz = (hslto.xyz * hslto.z) + (hslt.xyz * (hslto.z-1));
    hslt.w = hslto.w;
